...
def train_once(
    ...
    val_edge_index = sampler.val_data.edge_label_index.cuda()
    val_edge_label = sampler.val_data.edge_label.cuda()
    ...
    for epoch in range(100):
        ...
        pred = (output > 0).int()
        train_acc = (pred == train_label).float().mean()

        model.eval()
        with torch.no_grad():
            val_embedding = model.encode(val_features, val_adj)
            val_output = model.decode(val_embedding, val_edge_index).view(-1)
        val_loss = F.binary_cross_entropy_with_logits(val_output, val_edge_label)
        val_loss_list.append(val_loss.item())
        val_auc = metrics.roc_auc_score(val_edge_label.cpu(), val_output.cpu())
